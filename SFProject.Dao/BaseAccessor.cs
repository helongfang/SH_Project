...
using SFProject.Exceptions;

namespace SFProject.Dao
{
    public class BaseAccessor
    {
        protected static readonly string TMSSqlConnection = ConfigurationManager.ConnectionStrings["TMS"].ConnectionString.ToString();
        ...
        private void AddParameterToDataBase(Database database, DbCommand command, params DbParam[] parameters)
        {
            if (parameters != null)
            {
                foreach (DbParam param in parameters)
                {
                    ...
                    {
                        _dataBase.AddOutParameter(command, param.Name, param.Type, param.Size);
                    }
                }
            }
        }
